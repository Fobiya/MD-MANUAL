...
if ( ! function_exists( 'wc_dropdown_variation_attribute_options' ) ) {
	...
	function wc_dropdown_variation_attribute_options( $args = array() ) {
		...
		$html  = '<!--woocommerce\inclides\wc-template-functions.php--><ul id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '" name="' . esc_attr( $name ) . '" data-attribute_name="attribute_' . esc_attr( sanitize_title( $attribute ) ) . '" data-show_option_none="' . ( $show_option_none ? 'yes' : 'no' ) . '">';
		$html .= '<li data-list="checkbox-images"><input type="radio" name="' . esc_attr( $id ) . '" value="">' . esc_html( $show_option_none_text ) . '</input></li>';
			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options, true ) ) {
						$html .= '<li data-list="checkbox-'.   $numbber++  .  '"><input type="radio" name="' . esc_attr( $id ) . '" value="' . esc_attr( $term->slug ) . '" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name, $term, $attribute, $product ) ) . '</input></li>';
					}
				}
			} else {
                   $numbber = 1;
				foreach ( $options as $option ) {
					...
					$html    .= '<li data-list="checkbox-'.     $numbber++   .  '"><input type="radio" name="' . esc_attr( $id ) . '" value="' . esc_attr( $option ) . '" ' . $selected . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $option, null, $attribute, $product ) ) . '</input></li>';
				}
			}
		}

		$html .= '</ul><!--woocommerce\inclides\wc-template-functions.php-->';
